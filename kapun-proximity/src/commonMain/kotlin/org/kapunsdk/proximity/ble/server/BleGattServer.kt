...
internal class ChunkAccumulator<K> {
	...
	fun consume(key: K, packet: ByteArray): ChunkProcessingResult {
		...
		return when (header) {
			0x00 -> {
				...
				val buffer = buffers.getOrPut(key) {
					// Single-chunk message: no 0x01 packet preceded it, so report its start here.
					Logger(TAG).debug("rx first chunk: char=$key bytes=${payload.size}")
					ChunkBuffer()
				}
				buffer.append(payload)
				val chunks = buffer.chunkCount
				val complete = buffer.takeAll()
				buffers.remove(key)
				Logger(TAG).debug("rx complete: char=$key chunks=$chunks bytes=${complete.size}")
	...
	private class ChunkBuffer {
		...
		/** Number of chunks appended so far, reported on completion to derive BLE throughput. */
