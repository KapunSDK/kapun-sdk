...
abstract class TransportProtocol(
	...
	companion object {
		private const val TAG = "TransportProtocol"
	...
	protected fun reportMessageReceived(data: ByteArray) {
		...
		Logger(TAG).debug("message assembled: bytes=${data.size} queueDepth=${messageReceivedQueue.size}")
		if (!inhibitCallbacks) {
