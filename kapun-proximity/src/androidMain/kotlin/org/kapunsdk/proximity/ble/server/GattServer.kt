...
internal class GattServer(
	...
	private val advertiserCallback = object : AdvertiseCallback() {
		...
		override fun onStartFailure(errorCode: Int) {
			Logger(TAG).error("advertising failed: errorCode=$errorCode")
	...
	override fun start(characteristics: List<BleGattCharacteristic>): Boolean {
		...
		try {
			Logger(TAG).debug("gatt server started: service=$serviceUuid characteristics=${characteristics.size}")
	...
	override fun startAdvertising(listener: BleAdvertiserListener) {
		...
		try {
			...
			Logger(TAG).debug("advertising started: service=$serviceUuid")
	...
	override fun onConnectionStateChange(device: BluetoothDevice, status: Int, newState: Int) {
		...
		when (newState) {
			BluetoothProfile.STATE_CONNECTED -> {
				Logger(TAG).debug("peer connected: status=$status")
			...
			BluetoothProfile.STATE_DISCONNECTED -> {
				Logger(TAG).debug("peer disconnected: status=$status")
	...
	private fun drainWritingQueue(charUuid: UUID) {
		...
		if (chunk.isShutdownMessage()) {
			...
			try {
				...
				Logger(TAG).debug(
					"tx chunk: char=$charUuid bytes=${chunk.payloadSize} queued=${writingQueues[charUuid]?.size ?: 0}"
				)
			} catch (e: SecurityException) {
				reportError(ProximityError.Unknown(e.message ?: e::class.simpleName ?: "Unknown error"))
			}
