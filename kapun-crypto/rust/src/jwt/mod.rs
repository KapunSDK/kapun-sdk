...
pub fn validate_sd_jwt_with_did_document(
    ...
    // vc_type is logged because callers retry this with a second type on a TypeError; without it
    // the retry looks like a duplicate validation of the same credential.
    log_debug!(
        "VALIDATER",
        &format!("sd-jwt validation started: vc_type={vc_type} kid={kid}")
    );
    ...
    log_debug!(
        "VALIDATER",
        &format!("sd-jwt validation succeeded: vc_type={vc_type}")
    );
    true
}
