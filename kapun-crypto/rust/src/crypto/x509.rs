...
mod tests {
    ...
    use kapun_x509::x509_parser::time::ASN1Time;

    use crate::{base64_decode, jwt::get_x509_from_jwt};

    use super::extract_certs;
    ...
    fn test_x509_chain() {
        ...
        let chain = certs
            ...
            .map(|a| a.original_cert)
            .collect::<Vec<_>>();
        assert!(kapun_x509::x509::verify_chain_at::<JosekitCryptoProvider>(
            chain,
            ASN1Time::from_timestamp(1746524963).unwrap(),
            true,
            true,
        ));
    }
